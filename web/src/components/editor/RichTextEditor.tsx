...
// Same class names the backoffice editor bakes into saved HTML, so old and new
// rows render identically. Kept in sync with RichTextContent.
const theme = {
  ...
  text: {
    ...
    code: 'rounded bg-surface-alt px-1 py-0.5 font-mono text-[12px]',
  },
  heading: {
  ...
  tableCellHeader: 'bg-surface-alt font-semibold',
}
...
export function RichTextEditor({
  ...
  return (
    <div className="overflow-hidden rounded-md border border-line bg-surface transition focus-within:border-brand">
      <LexicalComposer
        initialConfig={{
          namespace: 'rte',
          theme,
      ...
      >
        ...
        <div className="relative">
          <RichTextPlugin
            contentEditable={
              <ContentEditable
                className="max-h-[360px] overflow-auto px-3 py-2 text-left text-sm leading-relaxed text-ink outline-none [&_table]:block [&_table]:overflow-auto"
                style={{ minHeight }}
              />
            }
            placeholder={
              placeholder ? (
                <div className="pointer-events-none absolute left-3 top-2 text-sm text-muted-400">
                  {placeholder}
                </div>
              ) : null
            }
